...
function authHeaders(extra={}){const headers={...extra};try{const token=sessionStorage.getItem("admin_token")||"";if(token)headers.Authorization=`Bearer ${token}`}catch{}return headers}
async function apiGet(path){const res=await fetchWithRetry(apiUrl(path),{cache:"no-store",headers:authHeaders()});const text=await res.text();let data;try{data=JSON.parse(text)}catch{data=text}if(!res.ok)throw new Error(data?.message||text||`HTTP ${res.status}`);return data}
async function apiPost(path,payload){const res=await fetchWithRetry(apiUrl(path),{method:"POST",headers:authHeaders({"Content-Type":"application/json"}),body:JSON.stringify(payload||{})});const text=await res.text();let data;try{data=JSON.parse(text)}catch{data=text}if(!res.ok)throw new Error(data?.message||text||`HTTP ${res.status}`);return data}
