...
const DAYS_PER_PAGE=7, MAX_DAYS=30, NORMAL_START=6, NORMAL_END=21, FULL_START=0, FULL_END=23, SAME_DAY_BLOCK_HOURS=3;

function pad(v){return String(v).padStart(2,"0")}
function formatDate(d){const x=new Date(d);return `${x.getFullYear()}-${pad(x.getMonth()+1)}-${pad(x.getDate())}`}
...
function makeDates(){
  ...
  const count=Math.min(DAYS_PER_PAGE,Math.max(0,MAX_DAYS-startIndex));
...
function applyGridColumns(grid,daysCount){
  ...
  grid.style.gridTemplateColumns=isMobile?`${timeCol}px repeat(${daysCount}, minmax(54px, 1fr))`:`${timeCol}px repeat(${daysCount}, minmax(105px, 1fr))`;
}
...
function renderCalendar(){
  ...
  grid.innerHTML="";
  if(!dates.length){range.textContent="";return}
...
document.addEventListener("DOMContentLoaded",()=>{
  document.getElementById("prevWeek").addEventListener("click",()=>{calendarState.page=Math.max(0,calendarState.page-1);loadCalendarData()});
  document.getElementById("nextWeek").addEventListener("click",()=>{calendarState.page+=1;loadCalendarData()});
  document.getElementById("modeToggleBtn").addEventListener("click",()=>{calendarState.fullDay=!calendarState.fullDay;renderCalendar()});
  loadCalendarData();
});
