...
function isMeterDriverAuthorized(request, env) {
...
__name(isMeterDriverAuthorized, "isMeterDriverAuthorized");
function parseDriverTenantHeaders(request) {
...
__name(parseDriverTenantHeaders, "parseDriverTenantHeaders");

// driver-reservations.js
var DRIVER_RESERVATIONS_PREFIX = "/api/driver/reservations/";
function parseDriverReservationIdFromPath(pathname) {
...
__name(parseDriverReservationIdFromPath, "parseDriverReservationIdFromPath");
function isDriverFixedFareReservation(row) {
  if (!row) {
    return false;
  }
...
__name(isDriverFixedFareReservation, "isDriverFixedFareReservation");
...
__name(buildScheduledAt, "buildScheduledAt");
...
__name(parseEstimateConsent, "parseEstimateConsent");
function buildConsentSummary(row, consentRow) {
  const inlineConsent = parseEstimateConsent(row?.estimate_consent);
  const consentAt = String(row?.consent_at || "").trim() || String(inlineConsent?.consentAt || inlineConsent?.agreedAt || "").trim() || String(consentRow?.consent_at || "").trim() || null;
  if (!consentAt && !inlineConsent && !consentRow) {
    return null;
  }
  const snapshotHash = String(inlineConsent?.snapshotHash || "").trim() || String(row?.quote_snapshot_hash || "").trim() || String(consentRow?.snapshot_hash || "").trim() || "";
  return {
    consentAt,
    consentTextVersion: String(inlineConsent?.consentTextVersion || consentRow?.consent_text_version || "").trim() || null,
    snapshotHash: snapshotHash || null,
    quotedFareYen: Number(inlineConsent?.quotedFare) || Number(row?.confirmed_fare) || Number(consentRow?.quoted_fare) || 0,
    source: consentRow ? "quote_consents" : inlineConsent ? "reservation" : "reservation_columns"
  };
}
__name(buildConsentSummary, "buildConsentSummary");
async function buildReservationIntegrity(row, snapshot, consentRow) {
  ...
  const confirmedFareMatchesSnapshot = Boolean(
    confirmedFareYen > 0 && derivedTotal > 0 && Math.abs(confirmedFareYen - derivedTotal) <= 1
  );
  const consentStoredHash = String(consentRow?.snapshot_hash || "").trim();
  const consentSnapshotHashMatches = consentStoredHash ? consentStoredHash === storedHash : null;
  return {
...
__name(buildReservationIntegrity, "buildReservationIntegrity");
...
__name(buildDriverReservationListItem, "buildDriverReservationListItem");
...
__name(buildDriverReservationDetail, "buildDriverReservationDetail");
...
__name(buildDriverReservationQuery, "buildDriverReservationQuery");
async function listDriverReservations(db, { date, franchiseeId = "", storeId = "" } = {}) {
  const query = buildDriverReservationQuery({ franchiseeId, storeId });
  const rows = await db.prepare(query.sql).bind(date, ...query.binds).all();
  return (rows.results || []).filter(isDriverFixedFareReservation).map(buildDriverReservationListItem);
}
__name(listDriverReservations, "listDriverReservations");
...
__name(fetchLatestQuoteConsent, "fetchLatestQuoteConsent");
async function getDriverReservationDetail(db, reservationId, tenant = {}) {
  const row = await db.prepare(`SELECT * FROM reservations WHERE id = ? LIMIT 1`).bind(reservationId).first();
  if (!row) {
...
__name(getDriverReservationDetail, "getDriverReservationDetail");
...
var worker_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/api/driver/reservations" && request.method === "GET") {
        return handleDriverListReservations(request, env, headers, url);
      }
      if (path.startsWith("/api/driver/reservations/") && request.method === "GET") {
        return handleDriverReservationDetail(request, env, headers, path);
      }
...
async function buildCorsHeaders(request, db) {
  ...
  return { "Content-Type": "application/json; charset=UTF-8", "Access-Control-Allow-Origin": allowOrigin, "Vary": "Origin", "Access-Control-Allow-Methods": "GET,POST,OPTIONS", "Access-Control-Allow-Headers": "Content-Type, Authorization, X-Franchisee-Id, X-Store-Id" };
}
__name(buildCorsHeaders, "buildCorsHeaders");
function bearerToken2(request) {
  const h = request.headers.get("Authorization") || "";
  const m = h.match(/^Bearer\s+(.+)$/i);
  return m ? m[1].trim() : "";
}
__name(bearerToken2, "bearerToken");
async function isAdminAuthorized(request, db) {
  const token = bearerToken2(request);
...
function parseEstimateTotalFromBody(body) {
  ...
  const snapshot = parseQuoteSnapshotFromBody(body);
  const fixedTotal = Number(snapshot?.fixedFareTotal) || 0;
  return fixedTotal + sumServiceFeesForTotal2(snapshot?.serviceFees);
}
...
async function isLpRegisterAuthorized(request, env, db) {
  const expected = String(env.LP_REGISTER_TOKEN || "").trim(), token = bearerToken2(request);
...
function calculateTotalFromSnapshot2(snapshot) {
  const fixedTotal = Number(snapshot?.fixedFareTotal) || 0;
  const derived = fixedTotal + sumServiceFeesForTotal2(snapshot?.serviceFees);
  const explicit = Number(snapshot?.total) || 0;
  if (derived > 0) return derived;
  if (explicit > 0) return explicit;
  return 0;
}
__name(calculateTotalFromSnapshot2, "calculateTotalFromSnapshot");
...
function validateRegisterPayload(body) {
  ...
  const derivedTotal = calculateTotalFromSnapshot2(snapshot);
...
async function handleDriverListReservations(request, env, headers, url) {
  if (!await isMeterDriverAuthorized(request, env)) return json({ success: false, message: "Unauthorized" }, 401, headers);
  const date = normalizeDate(url.searchParams.get("date") || "");
  if (!date) return json({ success: false, message: "date \u304C\u5FC5\u8981\u3067\u3059 (YYYY-MM-DD)" }, 400, headers);
  const tenant = parseDriverTenantHeaders(request);
  const reservations = await listDriverReservations(env.DB, { date, franchiseeId: tenant.franchiseeId, storeId: tenant.storeId });
  return json({ success: true, date, reservations }, 200, headers);
}
__name(handleDriverListReservations, "handleDriverListReservations");
async function handleDriverReservationDetail(request, env, headers, path) {
  if (!await isMeterDriverAuthorized(request, env)) return json({ success: false, message: "Unauthorized" }, 401, headers);
  const reservationId = parseDriverReservationIdFromPath(path);
  if (!reservationId) return json({ success: false, message: "\u4E88\u7D04ID\u304C\u4E0D\u6B63\u3067\u3059" }, 400, headers);
  const tenant = parseDriverTenantHeaders(request);
  const result = await getDriverReservationDetail(env.DB, reservationId, tenant);
  if (!result.ok) return json({ success: false, message: result.message }, result.status, headers);
  return json({ success: true, reservation: result.reservation }, 200, headers);
}
__name(handleDriverReservationDetail, "handleDriverReservationDetail");
export {
