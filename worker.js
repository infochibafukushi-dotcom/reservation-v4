export default {
  async fetch(request, env) {
    ...
    if (path === "/api/createReservation" && request.method === "POST") {
      ...
      const roundTrip = String(body.roundTrip || body.type || "").trim();
    ...
    if (path === "/api/deleteReservation" && request.method === "POST") {
      ...
      if (!id) {
        return new Response(JSON.stringify({ success: false, message: "idなし" }), { headers });
      }
      ...
      if (!reservation) {
        return new Response(JSON.stringify({ success: false, message: "予約なし" }), { headers });
      }
      ...
      const [datePart, timePart] = String(reservation.reservation_datetime || "").split(" ");
