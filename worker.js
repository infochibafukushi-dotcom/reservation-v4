  ...
  async fetch(request, env) {
    ...
    try{
      ...
      if(path==="/api/getBlocks"){
        const blocks=await env.DB.prepare(`SELECT id,date,time,type FROM blocks ORDER BY date,time`).all();
        return json({success:true,blocks:blocks.results||[]},200,headers);
      }

      if(path==="/api/getReservations"){
        const rows=await env.DB.prepare(`SELECT * FROM reservations ORDER BY created_at DESC,id DESC`).all();
      ...
      if(path==="/api/getUITexts"){
        return json({uiTexts:{index_title:"介護タクシー予約",index_subtitle:"丁寧・安全な送迎をご提供します"}},200,headers);
      }

      if(path==="/api/baseFees"){
        return json({baseFees:{items:[{id:"base",label:"基本運賃",price:2000,visible:true},{id:"dispatch",label:"予約配車料",price:500,visible:true}],note:"走行距離・待機時間・追加介助により最終金額は変動する場合があります。"}},200,headers);
      }
      ...
      if(path==="/api/createReservation"&&request.method==="POST"){
        const body=await safeJson(request);
        for(const key of["name","phone","date","time","pickup"]){
          if(!String(body[key]||"").trim())return json({success:false,message:"必須項目が未入力です"},400,headers);
        }
        ...
        for(const slot of slots){
          ...
          if(exists)return json({success:false,message:"この枠は予約できません"},409,headers);
        }

        const id=Date.now().toString();

        await env.DB.prepare(`INSERT INTO reservations (id,name,phone,date,time,pickup,destination,vehicle,assist,stairs,roundTrip,notes,estimate,block_count,status,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
          .bind(id,String(body.name||""),String(body.phone||""),date,time,String(body.pickup||""),String(body.destination||""),String(body.vehicle||""),String(body.assist||""),String(body.stairs||""),String(body.roundTrip||""),String(body.notes||""),String(body.estimate||""),blockCount,"active",new Date().toISOString()).run();

        for(const slot of slots){
          await env.DB.prepare(`INSERT OR IGNORE INTO blocks (date,time,type,reservation_id,created_at) VALUES (?,?,?,?,?)`)
      ...
      if(path==="/api/admin/sales/summary"){
        const rows=await env.DB.prepare(`SELECT estimate,status FROM reservations`).all();
        const list=rows.results||[];
        const sales=list.reduce((s,r)=>s+Number(String(r.estimate||"0").replace(/[^\d]/g,"")||0),0);
        return json({count:list.length,sales},200,headers);
      }
      ...
      if(path==="/api/admin/reservations/csv"){
        ...
        return new Response(csv,{headers:{"Content-Type":"text/csv; charset=UTF-8","Content-Disposition":"attachment; filename=reservations.csv","Access-Control-Allow-Origin":"*"}});
      }

      if(path==="/api/admin/settings"){
        const notify=await getSetting(env.DB,"notify_webhook_url","");
        return json({success:true,settings:{notify_webhook_url:notify}},200,headers);
      }

      if(path==="/api/admin/settings/save"&&request.method==="POST"){
        const body=await safeJson(request);
        await setSetting(env.DB,"notify_webhook_url",String(body.notify_webhook_url||""));
        if(String(body.new_password||"").trim())await setSetting(env.DB,"admin_password",String(body.new_password).trim());
        return json({success:true},200,headers);
      }
      ...
      if(path==="/api/admin/blocks/slot"&&request.method==="POST"){
        ...
        else await env.DB.prepare(`INSERT OR IGNORE INTO blocks (date,time,type,reservation_id,created_at) VALUES (?,?,'manual','',?)`).bind(date,time,new Date().toISOString()).run();
        return json({success:true},200,headers);
      }
      ...
      if(path==="/api/admin/blocks/day"&&request.method==="POST"){
        ...
        const manual=await env.DB.prepare(`SELECT COUNT(*) AS c FROM blocks WHERE date=? AND type='manual'`).bind(date).first();
        const shouldUnblock=String(body.mode||"")==="unblock"||(String(body.mode||"")==="toggle"&&Number(manual?.c||0)>=times.length);
        if(shouldUnblock)await env.DB.prepare(`DELETE FROM blocks WHERE date=? AND type='manual'`).bind(date).run();
        else for(const time of times)await env.DB.prepare(`INSERT OR IGNORE INTO blocks (date,time,type,reservation_id,created_at) VALUES (?,?,'manual','',?)`).bind(date,time,new Date().toISOString()).run();
        return json({success:true},200,headers);
      }
...
async function ensureSchema(db){
  ...
  for(const [c,t] of Object.entries({
    name:"TEXT",phone:"TEXT",date:"TEXT",time:"TEXT",pickup:"TEXT",destination:"TEXT",
    vehicle:"TEXT",assist:"TEXT",stairs:"TEXT",roundTrip:"TEXT",notes:"TEXT",estimate:"TEXT",
    block_count:"INTEGER DEFAULT 2",status:"TEXT DEFAULT 'active'",created_at:"TEXT"
  })) await addColumnIfMissing(db,"reservations",c,t);
...
async function getMenu(db){
  ...
  if(saved){
    try{
      ...
      items.forEach(i=>{if(grouped[i.group])grouped[i.group].push({name:i.name,price:Number(i.price||0)})});
      return grouped;
    }catch{}
  }
  return {
...
function toCsv(rows){
  const cols=["id","name","phone","date","time","pickup","destination","vehicle","assist","stairs","roundTrip","estimate","status","created_at"];
...
async function notify(db,body,id){
  ...
  if(!url)return;
  try{
    await fetch(url,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"reservation_created",id,body})});
  }catch{}
}
