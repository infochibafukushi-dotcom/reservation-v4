...
### 8-1. 実施内容
- JS主要ファイルの構文チェック（`node --check`）。
- エンドポイント参照の全探索（`rg`）で API 名の不整合を検出。
- 公開/管理/旧ページの導線差分を確認。

### 8-2. 発見した不整合・潜在ERROR
1. **旧ページ系の UI Texts エンドポイント参照ミス**
   - `app.js` と `form.html` が `ENDPOINTS.getUITexts` を参照しているが、`config.js` の定義は `uiTexts`（`getUITexts` は未定義）。
   - 影響: 旧ページ導線では UI文言取得URLが `undefined` になり、文言反映が失敗する可能性。

2. **旧資産と現行資産の二重運用リスク**
   - 現行は `index.*.js` + `admin.js` だが、`app.js` / `form.html` が同居。
   - 影響: 「どのページが本番採用か」が曖昧な環境では、修正漏れや再発の温床。

3. **`/api/getReservations` が管理系権限を要求する設計差**
   - URLは公開系に見えるが実装は認証必須。
   - 影響: フロントが無認証想定で呼ぶと 401。命名上の誤解リスクあり。

### 8-3. ERROR再発防止に向けた即時アクション
1. `config.js` の命名に合わせて旧ページ参照を `ENDPOINTS.uiTexts` に統一。
2. 旧ページ群（`app.js`, `form.html`, `calendar.js`）を `legacy/` へ隔離。
3. 管理系 API はパス命名を `/api/admin/...` へ統一し、用途を明確化。
4. デプロイ前チェックとして「エンドポイント未定義参照検査」を CI に追加。

### 8-4. 「100%検証」について
- 本レポートは**静的解析+コード読解ベースで可能な範囲を最大化**した結果です。
- ただし、Cloudflare本番設定値（環境変数/実DBデータ/実配信キャッシュ）を含む**実環境動作の100%保証**は、統合テスト環境と本番ログ確認なしには理論上不可能です。
