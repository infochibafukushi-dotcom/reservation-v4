...
function mapHandoffToFormValues(handoff) {
  ...
  let roundTrip = mapId(TRIP_MAP, selections.tripTypeId, handoff, [USAGE_LABELS.trip, "送迎方法"]);
  if (selections.tripTypeId === "round-trip" && selections.roundTripAddonId) {
    const addon = mapId(ROUND_ADDON_MAP, selections.roundTripAddonId, handoff, [USAGE_LABELS.roundTripAddon, "待機・付き添い"]);
    if (addon) roundTrip = addon;
  }
  return {
    pickup,
    destination,
