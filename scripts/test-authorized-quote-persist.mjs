...
assertEqual(Number(estimate.total), 7762, "estimate screen total");
assertEqual(Number(snap.preFixedFareAmount), 4862, "preFixedFareAmount");
assertEqual(Number(snap.totalAmount), 7762, "quoteSnapshot.totalAmount");
...
assertEqual(Number(normalized.fixedFareTotal), Number(snap.fixedFareTotal), "register keeps 1-yen precision");
assertEqual(Number(normalized.totalAmount), 7762, "quotes.total_amount candidate");
assertEqual(Number(normalized.total), 7762, "confirmed_fare candidate");

// 過去データ互換: 係数 null のスナップショットも読める
...
assertEqual(Number(legacy.total), 7762, "legacy null-coefficient snapshot readable");
